...
def create_fractal(xmin, xmax, ymin, ymax, width, height):
    ...
    # Функция np.meshgrid принимает два одномерных массива x и y и создает две матрицы
    # np.array(...) оборачивает результат meshgrid в массив NumP
    # .T транспонирует полученный массив
    # В результате выполнения этой строки кода получается двумерный массив, где каждая
    # строка содержит координаты всех возможных комбинаций значений из массивов x и y
    Z = Z[:, 0] + 1j * Z[:, 1]
